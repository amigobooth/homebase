...
loop do
  ...
  disconnected_user_ids.each do |uid|
    ...
    puts "Marking device #{device["hardware_address"]} disconnected"
    http_client.post "https://amigobooth.com/api/v1/services/devices/#{device["hardware_address"]}/disconnect"
  end
  ...
  entries.each do |e|
    ...
    if e.user_id.between?(2001, 2999)
      ...
      puts "Marking device #{device["hardware_address"]} connected"
      http_client.post "https://amigobooth.com/api/v1/services/devices/#{device["hardware_address"]}/connect"
